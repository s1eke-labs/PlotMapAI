...
export default function CharacterGraphPage() {
  ...
  const getViewportPointRef = useRef(getViewportPoint);
  getViewportPointRef.current = getViewportPoint;

  const handleWheel = useCallback((event: WheelEvent) => {
    const viewportPoint = getViewportPointRef.current(event.clientX, event.clientY);
    if (!viewportPoint) return;
