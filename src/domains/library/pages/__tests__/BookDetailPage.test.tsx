...
import { analysisApi, type AnalysisChunkStatus, type AnalysisStatusResponse } from '@domains/analysis';
...
describe('BookDetailPage', () => {
  beforeEach(() => {
    ...
    vi.mocked(libraryApi.get).mockResolvedValue(baseNovel);
    vi.mocked(libraryApi.getCoverUrl).mockResolvedValue('blob:cover');
    vi.mocked(libraryApi.delete).mockResolvedValue({ message: 'Novel deleted' });
  ...
  it('falls back to TxtCover when the novel has no stored cover', async () => {
    vi.mocked(libraryApi.get).mockResolvedValueOnce(baseNovel);
    renderPage();
  ...
  it('deletes the novel after confirming in the modal', async () => {
    ...
    await waitFor(() => {
      expect(libraryApi.delete).toHaveBeenCalledWith(1);
    });
