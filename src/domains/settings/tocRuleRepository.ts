...
export const tocRuleRepository = {
  ...
  uploadTocRulesYaml: async (file: File): Promise<TocRule[]> => {
    ...
    return tocRuleRepository.getTocRules();
  },
  ...
  getEnabledChapterDetectionRules: async (): Promise<ChapterDetectionRule[]> => {
    const rules = await db.tocRules.filter((rule) => rule.enable).sortBy('serialNumber');
